...
from moon_toe import available_moves, tuple_move_to_flat
import numpy as np
...
def get_stochastic_network_move(session, input_layer, output_layer, board_state, side,
    ...
    if valid_only:
        available_moves1 = list(available_moves(board_state))
        if len(available_moves1) == 1:
            move = np.zeros(100)
            np.put(move, tuple_move_to_flat(available_moves1[0]), 1)
            return move
        available_moves_flat = [tuple_move_to_flat(x) for x in available_moves1]
        for i in range(100):
